...
return String(text)
.replace(/&#x([0-9a-fA-F]+);/g, (, hex) => String.fromCodePoint(parseInt(hex, 16)))
.replace(/&#(\d+);/g, (, num) => String.fromCodePoint(parseInt(num, 10)))
.replace(/"/gi, '"')
.replace(/'/gi, "'")
.replace(/'/gi, "'")
.replace(/&/gi, '&')
...
const title = getMeta(html, 'og') || getMeta(html, 'twitter') || '';
const description = getMeta(html, 'og') || getMeta(html, 'twitter') || '';
const image = getMeta(html, 'og') || getMeta(html, 'twitter') || '';
const ogUrl = getMeta(html, 'og') || '';
