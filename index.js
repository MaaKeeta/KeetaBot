...
const IG_DOC_ID =
    process.env.IG_DOC_ID || '25531498899829322';
...
function getDateStr() {
    ...
    return `${day}/${month}/${parseInt(yearCE, 10) + 543}`;
}
...
async function fetchFacebookMetadata(
    ...
    try {
        ...
        if (!response.ok) {
            console.log(
                `Facebed ตอบ ${response.status}: ${proxyUrl}`
            );
...
/*
 * สำคัญ:
 * ไม่ log debug ทุกบรรทัด เพราะ discord.js
 * บาง debug message มี token อยู่ในข้อความ
 */
client.on('debug', info => {
    ...
    if (
        /gateway|connect|disconnect|resume|reconnect|identify|heartbeat|session/i.test(
            text
        )
    ) {
        console.log(
...
client.on(
    ...
    (event, shardId) => {
        console.error(
            `[SHARD DISCONNECT] shard=${shardId} code=${event?.code} reason=${event?.reason || 'ไม่มี reason'}`
        );
    }
);
...
/* =========================================================
   DISCORD NETWORK TEST
========================================================= */

async function testDiscordNetwork() {
    console.log(
        '🌐 กำลังทดสอบการเชื่อมต่อ Discord API...'
    );
    ...
    try {
        ...
        console.log(
            `[NETWORK TEST] Discord API status=${response.status} time=${elapsed}ms`
        );

        if (!response.ok) {
            console.error(
                '[NETWORK TEST] Discord API ตอบกลับไม่ปกติ'
            );
        ...
        if (data?.url) {
            console.log(
                `[NETWORK TEST] Gateway URL: ${data.url}`
            );
        } else {
            console.warn(
                '[NETWORK TEST] ได้ response แต่ไม่มี gateway URL'
            );
        }
        ...
        console.error(
            '[NETWORK TEST] Discord API FAILED:',
            error.message
        );
...
async function startDiscord() {
    ...
    if (
        ...
        console.log(
            `DISCORD_TOKEN: มีค่า (${process.env.DISCORD_TOKEN.length} ตัวอักษร)`
        );
    } else {
        console.error(
    ...
    const networkOK =
        await testDiscordNetwork();

    if (!networkOK) {
        console.error(
            '❌ Render ติดต่อ Discord API ไม่สำเร็จ'
        );

        console.error(
            'หยุด process เพื่อให้ Render restart ใหม่'
        );
    ...
    try {
        ...
        console.error(
            'กำลังปิด process เพื่อให้ Render restart...'
        );
